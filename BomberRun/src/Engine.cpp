...
Engine::Engine() :	running(true),
					...
					player()
{
	stateMachine = std::unique_ptr<StateMachine>(std::make_unique<StateMachine>(this));
	stateMachine->setCurrentState(std::make_shared<Title>(mediaCache, player));
}
...
void Engine::handleEvents()
{
	...
	while(SDL_PollEvent(&e))
	{
		...
		{
			changeState(std::make_shared<Title>(mediaCache, player));
		}
